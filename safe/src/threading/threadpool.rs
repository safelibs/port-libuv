...
// SAFETY(syscall_ffi): task state stays heap allocated and is synchronized by the pool queues.
unsafe impl Send for TaskState {}
// SAFETY(syscall_ffi): shared references only observe task state under the threadpool's synchronization rules.
...
// SAFETY(syscall_ffi): the pool mutex guards all shared access to the inner queues and counters.
...
// SAFETY(syscall_ffi): global pool control is accessed behind initialization and process-scope synchronization.
unsafe impl Sync for PoolControlCell {}

static THREADPOOL_CONTROL: PoolControlCell =
    PoolControlCell(std::cell::UnsafeCell::new(PoolControl {
        pid: 0,
        pool: null_mut(),
    }));
...
// SAFETY(syscall_ffi): these resolver entry points are raw libc APIs used by worker threads.
unsafe extern "C" {
...
#[inline]
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn set_req_state(req: *mut abi::uv_req_t, state: *mut TaskState) {
    unsafe {
        unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn init_lock() -> *mut abi::uv_mutex_t {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn queue_completion(state_ptr: *mut TaskState) {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn drain_loop_queue(loop_: *mut abi::uv_loop_t) {
    unsafe {
...
pub(crate) extern "C" fn loop_wq_async_cb(handle: *mut abi::uv_async_t) {
    unsafe {
        if handle.is_null() {
            return;
        }
...
// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn work_cb_trampoline(work: *mut abi::uv__work) {
    unsafe {
        let req = unsafe {
            work.cast::<u8>()
...
// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn work_done_trampoline(work: *mut abi::uv__work, status: c_int) {
    unsafe {
        let req = unsafe {
            work.cast::<u8>()
                .sub(offset_of!(abi::uv_work_t, work_req))
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn cancel(req: *mut abi::uv_req_t) -> c_int {
    unsafe {
        if req.is_null() {
            return abi::uv_errno_t_UV_EINVAL;
        }
...
// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn getaddrinfo_work(work: *mut abi::uv__work) {
    unsafe {
        let req = unsafe {
            work.cast::<u8>()
        ...
        let err =
            unsafe { c_getaddrinfo((*req).hostname, (*req).service, (*req).hints, &mut result) };
        unsafe {
            (*req).addrinfo = result;
...
// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn getaddrinfo_done(work: *mut abi::uv__work, status: c_int) {
    unsafe {
        let req = unsafe {
            work.cast::<u8>()
                .sub(offset_of!(abi::uv_getaddrinfo_t, work_req))
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn freeaddrinfo(ai: *mut abi::addrinfo) {
    unsafe {
        if !ai.is_null() {
            unsafe {
                c_freeaddrinfo(ai);
...
// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn getnameinfo_work(work: *mut abi::uv__work) {
    unsafe {
        let req = unsafe {
            work.cast::<u8>()
...
// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn getnameinfo_done(work: *mut abi::uv__work, status: c_int) {
    unsafe {
        let req = unsafe {
            work.cast::<u8>()
                .sub(offset_of!(abi::uv_getnameinfo_t, work_req))
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn cleanup_fs_allocations(req: *mut abi::uv_fs_t) {
    unsafe {
        for idx in 1..=3 {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn clone_path_slot(req: *mut abi::uv_fs_t, path: *const c_char, slot: usize) -> c_int {
    unsafe {
        ...
        match unsafe { (*req).fs_type } {
            abi::uv_fs_type_UV_FS_READ => {
                ...
                let rc = if off < 0 {
                    ...
                    let buf = unsafe { *(*req).bufs };
                    unsafe {
                        libc::pread((*req).file, buf.base.cast(), buf.len, off as libc::off_t)
                    }
                } else {
            ...
            abi::uv_fs_type_UV_FS_OPEN => {
                let rc =
                    unsafe { libc::open((*req).path, (*req).flags, (*req).mode as libc::mode_t) };
                unsafe {
                    if rc >= 0 {
            ...
            abi::uv_fs_type_UV_FS_WRITE => {
                ...
                let rc = if off < 0 {
                    ...
                    let buf = unsafe { *(*req).bufs };
                    unsafe {
                        libc::pwrite((*req).file, buf.base.cast(), buf.len, off as libc::off_t)
                    }
                } else {
