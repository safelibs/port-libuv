...
pub(crate) unsafe fn create_ex(
    ...
    let mut stack_size = if !params.is_null()
        && unsafe { (*params).flags & abi::uv_thread_create_flags_UV_THREAD_HAS_STACK_SIZE } != 0
    {
    ...
    let err = unsafe { libc::pthread_create(tid.cast(), attr_ptr.cast_const(), thread_start, raw) };
...
pub(crate) unsafe fn join(tid: *mut abi::uv_thread_t) -> c_int {
    if tid.is_null() {
        return abi::uv_errno_t_UV_EINVAL;
    }
...
pub(crate) unsafe fn self_thread() -> abi::uv_thread_t {
    unsafe { libc::pthread_self() }
}

pub(crate) unsafe fn equal(t1: *const abi::uv_thread_t, t2: *const abi::uv_thread_t) -> c_int {
...
pub(crate) unsafe fn setaffinity(
    tid: *mut abi::uv_thread_t,
    cpumask: *mut c_char,
    oldmask: *mut c_char,
    mask_size: usize,
...
pub(crate) unsafe fn getaffinity(
    tid: *mut abi::uv_thread_t,
    cpumask: *mut c_char,
    mask_size: usize,
) -> c_int {
...
pub(crate) unsafe fn getcpu() -> c_int {
...
pub(crate) unsafe fn getpriority(tid: abi::uv_thread_t, priority: *mut c_int) -> c_int {
    if priority.is_null() {
        return abi::uv_errno_t_UV_EINVAL;
    }
...
unsafe fn set_nice_for_calling_thread(priority: c_int) -> c_int {
...
pub(crate) unsafe fn setpriority(tid: abi::uv_thread_t, priority: c_int) -> c_int {
    if !(abi::UV_THREAD_PRIORITY_LOWEST..=abi::UV_THREAD_PRIORITY_HIGHEST).contains(&priority) {
        return abi::uv_errno_t_UV_EINVAL;
    }
