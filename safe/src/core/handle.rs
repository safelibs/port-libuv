...
// SAFETY(syscall_ffi): these stdio symbols are declared by libc and only used through the C ABI.
unsafe extern "C" {
...
pub(crate) fn is_active(handle: *const abi::uv_handle_t) -> bool {
    unsafe { !handle.is_null() && unsafe { ((*handle).flags & UV_HANDLE_ACTIVE) != 0 } }
}

#[inline]
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn is_closing(handle: *const abi::uv_handle_t) -> bool {
    unsafe {
        !handle.is_null()
...
#[inline]
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn has_ref(handle: *const abi::uv_handle_t) -> bool {
    unsafe { !handle.is_null() && unsafe { ((*handle).flags & UV_HANDLE_REF) != 0 } }
}

#[inline]
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn is_active_int(handle: *const abi::uv_handle_t) -> c_int {
    unsafe { unsafe { is_active(handle) as c_int } }
}

#[inline]
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn is_closing_int(handle: *const abi::uv_handle_t) -> c_int {
    unsafe { unsafe { is_closing(handle) as c_int } }
}

#[inline]
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn has_ref_int(handle: *const abi::uv_handle_t) -> c_int {
    unsafe { unsafe { has_ref(handle) as c_int } }
}

// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn handle_start(handle: *mut abi::uv_handle_t) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn handle_stop(handle: *mut abi::uv_handle_t) {
    unsafe {
        if unsafe { (*handle).flags & UV_HANDLE_ACTIVE } == 0 {
            return;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn handle_ref(handle: *mut abi::uv_handle_t) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn handle_unref(handle: *mut abi::uv_handle_t) {
    unsafe {
        if handle.is_null() || unsafe { (*handle).flags & UV_HANDLE_REF } == 0 {
            return;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn prepare_init(loop_: *mut abi::uv_loop_t, prepare: *mut abi::uv_prepare_t) -> c_int {
    unsafe {
        let rc = unsafe {
            handle_init(
                loop_,
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn prepare_start(prepare: *mut abi::uv_prepare_t, cb: abi::uv_prepare_cb) -> c_int {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn prepare_stop(prepare: *mut abi::uv_prepare_t) -> c_int {
    unsafe {
        if prepare.is_null() {
            return abi::uv_errno_t_UV_EINVAL;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn idle_init(loop_: *mut abi::uv_loop_t, idle: *mut abi::uv_idle_t) -> c_int {
    unsafe {
        let rc = unsafe {
            handle_init(
                loop_,
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn idle_start(idle: *mut abi::uv_idle_t, cb: abi::uv_idle_cb) -> c_int {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn idle_stop(idle: *mut abi::uv_idle_t) -> c_int {
    unsafe {
        if idle.is_null() {
            return abi::uv_errno_t_UV_EINVAL;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn run_prepare_handles(loop_: *mut abi::uv_loop_t) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn run_idle_handles(loop_: *mut abi::uv_loop_t) {
    unsafe {
        let mut local = abi::uv__queue::default();
        unsafe {
            queue::move_queue(
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn run_prepare(loop_: *mut abi::uv_loop_t) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn run_idle(loop_: *mut abi::uv_loop_t) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn type_label(handle: *mut abi::uv_handle_t) -> *const c_char {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn queue_close(loop_: *mut abi::uv_loop_t, handle: *mut abi::uv_handle_t) {
    unsafe {
        unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn close(handle: *mut abi::uv_handle_t, close_cb: abi::uv_close_cb) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn finish_close(handle: *mut abi::uv_handle_t, inner: &mut LoopStateInner) {
    unsafe {
        unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn run_closing_handles(loop_: *mut abi::uv_loop_t) {
    unsafe {
        let state = unsafe { &*loop_state(loop_) };
        let mut inner = state.inner.lock().unwrap();
