...
fn heap_swap(inner: &mut LoopStateInner, left: usize, right: usize) {
    unsafe {
        unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn heap_sift_up(inner: &mut LoopStateInner, mut index: usize) {
    unsafe {
        while index > 0 {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn heap_sift_down(inner: &mut LoopStateInner, mut index: usize) {
    unsafe {
        loop {
            let left = index * 2 + 1;
            let right = left + 1;
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn ensure_timer_capacity(inner: &mut LoopStateInner, loop_: *mut abi::uv_loop_t) -> bool {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn update_public_heap(loop_: *mut abi::uv_loop_t, inner: &LoopStateInner) {
    unsafe {
        unsafe {
            (*loop_).timer_heap.min = inner.timer_heap.cast();
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn heap_insert(loop_: *mut abi::uv_loop_t, entry: TimerHeapEntry) -> bool {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn heap_remove_active(loop_: *mut abi::uv_loop_t, handle_ptr: *mut abi::uv_timer_t) {
    unsafe {
        let state = unsafe { &*loop_state(loop_) };
        let mut inner = state.inner.lock().unwrap();
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn heap_pop_min(loop_: *mut abi::uv_loop_t) -> Option<TimerHeapEntry> {
    unsafe {
        let state = unsafe { &*loop_state(loop_) };
        let mut inner = state.inner.lock().unwrap();
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn heap_min(loop_: *mut abi::uv_loop_t) -> Option<TimerHeapEntry> {
    unsafe {
        let state = unsafe { &*loop_state(loop_) };
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn timer_init(loop_: *mut abi::uv_loop_t, handle_ptr: *mut abi::uv_timer_t) -> c_int {
    unsafe {
        let rc = unsafe {
            handle::handle_init(
                loop_,
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn timer_stop(handle_ptr: *mut abi::uv_timer_t) -> c_int {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn timer_again(handle_ptr: *mut abi::uv_timer_t) -> c_int {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn timer_get_due_in(handle_ptr: *const abi::uv_timer_t) -> u64 {
    unsafe {
        if handle_ptr.is_null() {
            return 0;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn next_timeout(loop_: *mut abi::uv_loop_t) -> c_int {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn run_timers(loop_: *mut abi::uv_loop_t) {
    unsafe {
        ...
        loop {
            ...
            let record = unsafe { find_handle_record(&inner, handle_ptr.cast()) };
            let stale =
                record.is_null() || unsafe { (*record).timer_generation != entry.generation };
            drop(inner);
            if stale {
                continue;
            }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn timer_close(handle_ptr: *mut abi::uv_timer_t) {
    unsafe {
        let _ = unsafe { timer_stop(handle_ptr) };
    }
}
