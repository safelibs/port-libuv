...
fn alloc_loop_state() -> *mut LoopState {
    unsafe { unsafe { allocator::alloc_value(LoopState::new()) } }
}

// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn update_time(loop_: *mut abi::uv_loop_t) {
    unsafe {
        if loop_.is_null() {
            return;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn now(loop_: *const abi::uv_loop_t) -> u64 {
    unsafe {
        if loop_.is_null() {
            return 0;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn backend_fd(loop_: *const abi::uv_loop_t) -> c_int {
    unsafe {
        if loop_.is_null() {
            return -1;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn free_loop_state(loop_: *mut abi::uv_loop_t) {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn loop_close(loop_: *mut abi::uv_loop_t) -> c_int {
    unsafe {
        if loop_.is_null() {
            return abi::uv_errno_t_UV_EINVAL;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn loop_new() -> *mut abi::uv_loop_t {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn loop_delete(loop_: *mut abi::uv_loop_t) {
    unsafe {
        if loop_.is_null() {
            return;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn loop_alive(loop_: *const abi::uv_loop_t) -> c_int {
    unsafe {
        if loop_.is_null() {
            return 0;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn backend_timeout_inner(loop_: *mut abi::uv_loop_t) -> c_int {
    unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn backend_timeout(loop_: *const abi::uv_loop_t) -> c_int {
    unsafe {
        if loop_.is_null() {
            return 0;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
fn wait_for_events(loop_: *mut abi::uv_loop_t, timeout_ms: c_int) {
    unsafe {
        if timeout_ms == 0 {
            return;
        }
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn stop(loop_: *mut abi::uv_loop_t) {
    unsafe {
        if !loop_.is_null() {
            unsafe {
                (*loop_).stop_flag = 1;
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn run(loop_: *mut abi::uv_loop_t, mode: abi::uv_run_mode) -> c_int {
    unsafe {
        if loop_.is_null() {
            return 0;
        }
        ...
        if mode == abi::uv_run_mode_UV_RUN_DEFAULT
            && alive != 0
            && unsafe { (*loop_).stop_flag } == 0
        {
            unsafe {
...
// SAFETY(abi_layout): accesses libuv's C ABI layout through raw pointers and stable field offsets.
pub(crate) fn metrics_idle_time(_loop_: *mut abi::uv_loop_t) -> u64 {
    unsafe { unsafe { metrics::metrics_idle_time(_loop_) } }
}
