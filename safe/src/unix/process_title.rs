...
unsafe fn set_process_name(title: *const c_char) {
    #[cfg(target_os = "linux")]
    unsafe {
...
pub(crate) unsafe fn setup_args(argc: c_int, argv: *mut *mut c_char) -> *mut *mut c_char {
...
pub(crate) unsafe fn set_process_title(title: *const c_char) -> c_int {
    if title.is_null() {
...
pub(crate) unsafe fn get_process_title(buffer: *mut c_char, size: usize) -> c_int {
