...
pub(crate) fn init(loop_: *mut abi::uv_loop_t, handle_: *mut abi::uv_fs_event_t) -> c_int {
    unsafe { unsafe { crate::unix::epoll::uv_fs_event_init(loop_.cast(), handle_.cast()) } }
}
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn stop(handle_: *mut abi::uv_fs_event_t) -> c_int {
    unsafe { unsafe { crate::unix::epoll::uv_fs_event_stop(handle_.cast()) } }
}
