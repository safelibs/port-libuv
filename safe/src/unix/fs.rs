...
unsafe fn free_slot(req: *mut abi::uv_fs_t, slot: usize) {
...
unsafe fn cleanup_owned_allocations(req: *mut abi::uv_fs_t) {
...
unsafe fn reset_request_pointers(req: *mut abi::uv_fs_t) {
    unsafe {
        (*req).path = null();
        (*req).new_path = null();
        (*req).bufs = null_mut();
...
unsafe fn fill_uv_statfs(out: *mut abi::uv_statfs_t, st: *const libc::statfs) {
    unsafe {
...
unsafe fn set_request_error(req: *mut abi::uv_fs_t, err: c_int) {
    unsafe {
...
unsafe fn clone_path_into(req: *mut abi::uv_fs_t, path: *const c_char, slot: usize) -> c_int {
...
unsafe fn finish_read_or_write(req: *mut abi::uv_fs_t) {
    unsafe {
...
unsafe fn scandir_entries(req: *mut abi::uv_fs_t) -> *mut abi::uv_dirent_t {
    unsafe { (*req).ptr.cast::<abi::uv_dirent_t>() }
}

unsafe fn readdir_cleanup(req: *mut abi::uv_fs_t) {
...
unsafe fn read_directory_entries(req: *mut abi::uv_fs_t) -> c_int {
...
unsafe fn readlink_pathmax(path: *const c_char) -> usize {
...
unsafe fn alloc_readlink_result(req: *mut abi::uv_fs_t) -> c_int {
...
unsafe fn alloc_realpath_result(req: *mut abi::uv_fs_t) -> c_int {
...
unsafe fn alloc_statfs_result(req: *mut abi::uv_fs_t) -> c_int {
...
unsafe fn run_opendir(req: *mut abi::uv_fs_t) -> c_int {
...
unsafe fn run_readdir(req: *mut abi::uv_fs_t) -> c_int {
    let dir = unsafe { (*req).ptr.cast::<abi::uv_dir_t>() };
    if dir.is_null() {
        return abi::uv_errno_t_UV_EINVAL;
    }
...
unsafe fn run_closedir(req: *mut abi::uv_fs_t) -> c_int {
    let dir = unsafe { (*req).ptr.cast::<abi::uv_dir_t>() };
    if dir.is_null() {
        return abi::uv_errno_t_UV_EINVAL;
    }
...
unsafe fn run_sendfile(req: *mut abi::uv_fs_t) -> isize {
...
unsafe fn run_read(req: *mut abi::uv_fs_t) -> isize {
    ...
    let rc = if off < 0 {
        ...
        unsafe { libc::preadv(file, bufs.as_ptr().cast(), nbufs as c_int, off as libc::off_t) }
    };
...
unsafe fn run_write_all(req: *mut abi::uv_fs_t) -> isize {
    let file = unsafe { (*req).file };
    let mut off = unsafe { (*req).off as i64 };
    let mut bufs = unsafe { slice::from_raw_parts((*req).bufs, (*req).nbufs as usize).to_vec() };
    ...
    while !bufs.is_empty() {
        ...
        let rc = if off < 0 {
            ...
            unsafe { libc::pwritev(file, bufs.as_ptr().cast(), chunk as c_int, off as libc::off_t) }
        };
        ...
        if rc <= 0 {
            ...
            if total == 0 {
                return if rc == 0 { 0 } else { uv_err(last_errno()) as isize };
            }
            return total;
        }
...
unsafe fn execute_fs(req: *mut abi::uv_fs_t) {
    let result = match unsafe { (*req).fs_type } {
        ...
        abi::uv_fs_type_UV_FS_CHOWN => {
            let rc = unsafe { libc::chown((*req).path, (*req).uid as libc::uid_t, (*req).gid as libc::gid_t) };
            if rc == 0 {
                0
            } else {
                uv_err(last_errno()) as isize
        ...
        abi::uv_fs_type_UV_FS_FCHOWN => {
            let rc = unsafe { libc::fchown((*req).file, (*req).uid as libc::uid_t, (*req).gid as libc::gid_t) };
            if rc == 0 {
                0
            } else {
                uv_err(last_errno()) as isize
        ...
        abi::uv_fs_type_UV_FS_LCHOWN => {
            let rc = unsafe { libc::lchown((*req).path, (*req).uid as libc::uid_t, (*req).gid as libc::gid_t) };
            if rc == 0 {
                0
            } else {
                uv_err(last_errno()) as isize
        ...
        abi::uv_fs_type_UV_FS_UTIME => {
            let ts = [fs_to_timespec(unsafe { (*req).atime }), fs_to_timespec(unsafe { (*req).mtime })];
...
unsafe fn run_or_submit(loop_: *mut abi::uv_loop_t, req: *mut abi::uv_fs_t) -> c_int {
...
pub(crate) unsafe fn req_cleanup(req: *mut abi::uv_fs_t) {
    if req.is_null() {
        return;
    }
    ...
    if unsafe { (*req).fs_type } == abi::uv_fs_type_UV_FS_READDIR && unsafe { !(*req).ptr.is_null() }
    {
        unsafe {
            readdir_cleanup(req);
        }
...
pub(crate) unsafe fn scandir_next(req: *mut abi::uv_fs_t, ent: *mut abi::uv_dirent_t) -> c_int {
...
unsafe fn init_two_path_request(
    loop_: *mut abi::uv_loop_t,
    req: *mut abi::uv_fs_t,
    fs_type: abi::uv_fs_type,
    path: *const c_char,
...
pub(crate) unsafe fn symlink(
    ...
    let rc = unsafe {
        init_two_path_request(loop_, req, abi::uv_fs_type_UV_FS_SYMLINK, path, new_path, cb)
    };
    if rc != 0 {
        return rc;
    }
