...
// SAFETY(syscall_ffi): the environment vector is a raw libc global used through the C ABI.
unsafe extern "C" {
    static mut environ: *mut *mut c_char;
}

// SAFETY(ffi_callback): bridges the libuv C ABI through raw pointers and callback types.
extern "C" fn child_watcher_cb(_handle: *mut abi::uv_signal_t, _signum: c_int) {
    unsafe {}
}
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn set_cloexec(fd: c_int, enabled: bool) -> c_int {
    unsafe {
        loop {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn write_child_error(error_fd: c_int) -> ! {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn init_handle(loop_: *mut abi::uv_loop_t, handle: *mut abi::uv_process_t) {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn init_spawn_stdio(container: *mut abi::uv_stdio_container_t, fds: &mut [c_int; 2]) -> c_int {
    unsafe {
        ...
        match flags & mask {
            ...
            abi::uv_stdio_flags_UV_CREATE_PIPE => {
                ...
                if stream.is_null()
                    || unsafe { (*stream).type_ } != abi::uv_handle_type_UV_NAMED_PIPE
                {
                    return abi::uv_errno_t_UV_EINVAL;
                }

                unsafe {
                    crate::unix::tcp::uv_socketpair(
                        libc::SOCK_STREAM,
                        0,
                        fds.as_mut_ptr().cast(),
                        0,
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn close_parent_spawn_stream(container: *mut abi::uv_stdio_container_t) {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn dup_stdio_fds(stdio_count: usize, pipes: &mut [[c_int; 2]], error_fd: c_int) {
    unsafe {
        for fd in 0..stdio_count {
            ...
            let duplicated =
                unsafe { libc::fcntl(use_fd, libc::F_DUPFD_CLOEXEC, stdio_count as c_int) };
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn reset_signal_dispositions(error_fd: c_int) {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn configure_child_stdio(stdio_count: usize, pipes: &mut [[c_int; 2]], error_fd: c_int) {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn close_child_pipe_fds(pipes: &[[c_int; 2]]) {
    unsafe {
        for pair in pipes {
            unsafe {
                if pair[0] > 2 {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn reset_signal_mask(error_fd: c_int) {
    unsafe {
        ...
        if unsafe { libc::pthread_sigmask(libc::SIG_SETMASK, mask.as_ptr(), std::ptr::null_mut()) }
            != 0
        {
            unsafe { write_child_error(error_fd) };
        }
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn apply_credential_drop(options: *const abi::uv_process_options_t, error_fd: c_int) {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
fn read_exec_error(error_fd: c_int, pid: c_int) -> c_int {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn loop_init(loop_: *mut abi::uv_loop_t) -> c_int {
    unsafe {
        if loop_.is_null() {
            return abi::uv_errno_t_UV_EINVAL;
        }
        ...
        let rc = unsafe {
            crate::unix::signal::init(loop_, std::ptr::addr_of_mut!((*loop_).child_watcher))
        };
        if rc != 0 {
            return rc;
        }
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn close(handle: *mut abi::uv_process_t) {
    unsafe {
        if handle.is_null() {
            return;
        }
        ...
        if !loop_.is_null()
            && unsafe { queue::is_empty(std::ptr::addr_of!((*loop_).process_handles)) }
        {
            let _ = unsafe {
                crate::unix::signal::stop(std::ptr::addr_of_mut!((*loop_).child_watcher))
            };
        }
    }
}
