...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn prepare_hostname(node: *const c_char) -> Result<*mut c_char, c_int> {
    unsafe {
        if node.is_null() {
            return Ok(null_mut());
        }
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn if_indextoname(ifindex: c_uint, buffer: *mut c_char, size: *mut usize) -> c_int {
    unsafe {
...
// SAFETY(syscall_ffi): crosses raw libc, kernel, or translated upstream FFI boundaries that Rust cannot model safely.
pub(crate) fn if_indextoiid(ifindex: c_uint, buffer: *mut c_char, size: *mut usize) -> c_int {
    unsafe { unsafe { if_indextoname(ifindex, buffer, size) } }
}
