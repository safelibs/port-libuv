...
unsafe fn ctx_path(ctx: *mut PollCtx) -> *const c_char {
    unsafe { ptr::addr_of!((*ctx).path).cast() }
}

unsafe fn poll_ctx_from_req(req: *mut abi::uv_fs_t) -> *mut PollCtx {
    unsafe {
        req.cast::<u8>()
...
unsafe fn poll_ctx_from_timer(timer: *mut abi::uv_timer_t) -> *mut PollCtx {
    unsafe {
        timer.cast::<u8>()
...
unsafe fn free_ctx(ctx: *mut PollCtx) {
    if ctx.is_null() {
        return;
    }
    unsafe {
...
unsafe fn statbuf_eq(a: *const abi::uv_stat_t, b: *const abi::uv_stat_t) -> bool {
    unsafe {
...
unsafe extern "C" fn timer_close_cb(timer: *mut abi::uv_handle_t) {
...
unsafe extern "C" fn poll_cb(req: *mut abi::uv_fs_t) {
    let ctx = unsafe { poll_ctx_from_req(req) };
    let handle = unsafe { (*ctx).parent_handle };
    if handle.is_null() {
        unsafe {
            fs::req_cleanup(req);
            epoll::close(ptr::addr_of_mut!((*ctx).timer_handle).cast(), Some(timer_close_cb));
        }
        return;
    }
        ...
        if unsafe { (*req).result } != 0 {
            if unsafe { (*ctx).busy_polling } != unsafe { (*req).result as c_int } {
                ...
                if let Some(cb) = unsafe { (*ctx).poll_cb } {
                    unsafe {
                        cb(handle, (*req).result as c_int, ptr::addr_of!((*ctx).statbuf), &zero);
                    }
                }
                unsafe {
    ...
    if !unsafe { handle::is_active(handle.cast()) } || unsafe { handle::is_closing(handle.cast()) } {
        unsafe {
            epoll::close(ptr::addr_of_mut!((*ctx).timer_handle).cast(), Some(timer_close_cb));
        }
        return;
...
unsafe extern "C" fn timer_cb(timer: *mut abi::uv_timer_t) {
...
pub(crate) unsafe fn init(loop_: *mut abi::uv_loop_t, handle_: *mut abi::uv_fs_poll_t) -> c_int {
...
pub(crate) unsafe fn start(
    ...
    unsafe {
        ...
        libc::memcpy(
            ptr::addr_of_mut!((*ctx).path).cast(),
            path.cast(),
            len + 1,
        );
    }

    let mut rc = unsafe { epoll::timer_init((*ctx).loop_, ptr::addr_of_mut!((*ctx).timer_handle)) };
    if rc != 0 {
        unsafe {
            free_ctx(ctx);
        }
...
pub(crate) unsafe fn stop(handle_: *mut abi::uv_fs_poll_t) -> c_int {
    if handle_.is_null() {
        return abi::uv_errno_t_UV_EINVAL;
    }
    if !unsafe { handle::is_active(handle_.cast()) } {
...
pub(crate) unsafe fn close(handle_: *mut abi::uv_fs_poll_t) {
    if handle_.is_null() {
        return;
    }
